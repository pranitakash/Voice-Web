...
export const Process = () => {
    return (
        <section className="py-32 px-4 bg-brand-background overflow-hidden">
            <div className="max-w-7xl mx-auto">
                <div className="text-center mb-20">
                    ...
                    <h2 className="text-5xl font-bold text-brand-foreground mb-6 tracking-tight">From thought to live site.</h2>
                    <p className="text-brand-foreground/40 text-xl max-w-2xl mx-auto">A seamless, interactive workflow designed for modern creators.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {steps.map((step, index) => (
                        <motion.div
                            ...
                            whileHover={{ y: -10 }}
                            className="relative group h-full rounded-[2.5rem] bg-brand-card border border-brand-border overflow-hidden flex flex-col p-2 shadow-2xl shadow-black/5 dark:shadow-none"
                        >
                            {/* Card Header: Image Container */}
                            <div className="relative h-64 rounded-[2rem] overflow-hidden mb-6 bg-brand-foreground/5">
                                <motion.img
                                    src={step.image}
                                    alt={step.title}
                                ...
                                <div className="absolute bottom-6 left-6 p-3 rounded-2xl bg-brand-background/60 backdrop-blur-xl border border-brand-border">
                                    <step.icon className={`w-6 h-6 ${step.color === 'text-brand-accent' ? 'text-brand-accent' : step.color}`} />
                                </div>
                            </div>
                            ...
                            <div className="px-8 pb-10 pt-2 flex-1 flex flex-col">
                                ...
                                <p className="text-brand-foreground/40 leading-relaxed font-medium text-sm">
                                    {step.description}
                                </p>
                            </div>
